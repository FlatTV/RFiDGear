...
#nullable disable
        ...
        // Note: upstream re-adds a CloseThreads(sender, CancelEventArgs) Closing handler here
        // that calls Environment.Exit(0) directly. Deliberately dropped: it would bypass
        // ConfirmCloseWithUnsavedChangesAsync (see MainWindow.xaml.cs's OnClosing), silently
        // discarding unsaved project changes on window close. Nothing in this codebase
        // subscribes to it anymore (that subscription line was removed in the same commit
        // that added the unsaved-changes prompt), so it would be dead code even if kept.
